...
{
    public class ProductionBaseRepository<T>: IBaseRepository<T> where T : class
    {
        ...
        {
            if (productionUnitOfWork == null) throw new ArgumentNullException("Product DbContext is not assigned");
        ...
        public bool Save()
        {
            ...
            return _productionUnitOfWork.Db.SaveChanges() > 0;
        }

        public async Task<bool> SaveAsync()
        {
            return await _productionUnitOfWork.Db.SaveChangesAsync() > 0;
        }
        ...
        {
            _productionUnitOfWork.Db.Entry(entity).State = System.Data.Entity.EntityState.Modified;
        }
        ...
        {
            using (var cmd = _productionUnitOfWork.Db.Database.Connection.CreateCommand())
            {
                cmd.CommandText = Sql;
